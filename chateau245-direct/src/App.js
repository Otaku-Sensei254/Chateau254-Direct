...
const storedSession = () => {
  try {
    return JSON.parse(localStorage.getItem('chateau254_session')) || null;
  } catch (error) {
    return null;
  }
};
...
const App = () => {
  ...
  const placeOrder = (event) => { event.preventDefault(); setOrder({ number: 'CH254-00124', total: subtotal + delivery }); navigate('/confirmation'); };
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000/api'}/auth/me`, { headers: { Authorization: `Bearer ${session.token}` } })
  ...
  return <div className="app-shell">
    {location.pathname !== '/' && location.pathname !== '/auth' && !location.pathname.startsWith('/admin') && location.pathname !== '/rider' && <AppHeader cartCount={cartCount} userName={session?.user?.full_name} onBack={handleBack} onCart={() => navigate('/cart')} onHome={() => navigate('/menu')} />}
    <Routes>
      ...
      <Route path="/admin/*" element={<ProtectedRoute user={session?.user} roles={['admin']}><AdminDashboard user={session?.user} onLogout={handleLogout} /></ProtectedRoute>} />
      <Route path="/rider/:riderId" element={<ProtectedRoute user={session?.user} roles={['rider']}><RiderDashboard user={session?.user} onLogout={handleLogout} /></ProtectedRoute>} />
