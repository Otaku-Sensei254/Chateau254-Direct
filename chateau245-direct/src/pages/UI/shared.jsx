...
const AppHeader = ({ cartCount, onBack, onCart, onHome, onProfile, userName = '' }) => <header className="app-header"><button className="back-button" aria-label="Go back" onClick={onBack}><FiArrowLeft /></button><button className="wordmark" onClick={onHome}><Brand /></button><div className="header-actions"><button className="welcome-link" onClick={onProfile}>Hi, {userName}</button><button className="bag-button" aria-label="Open cart" onClick={onCart}><FiShoppingBag />{cartCount > 0 && <b>{cartCount}</b>}</button></div></header>;
